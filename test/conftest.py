...
def pytest_bdd_apply_tag(tag, function):
    if tag == "flaky":
        # Just a marker, nothing more: the test runs once and reports its real
        # pass/fail status, so a genuine regression stays visible in Allure
        # instead of being muted. CI stays unblocked by running known_flaky
        # tests in their own continue-on-error step (see ci.yml), not by
        # faking the result here.
